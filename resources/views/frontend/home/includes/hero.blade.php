<section class="hero-area">
    <div class="hero-slider owl-action-styled">
        ...
        <div class="hero-slider-item" style="background-image: url({{ asset($slider->image) }}); background-repeat: no-repeat;">
                <div class="container">
                    <div class="hero-content">
                        <div class="section-heading">
                            <h2 class="section__title text-white fs-65 lh-80 pb-3">
                                @foreach (array_chunk(explode(' ', trim($slider->title)), 3) as $index => $chunk)
                                    {!! $loop->first ? '' : '<br>' !!}{{ e(implode(' ', $chunk)) }}
                                @endforeach
                            </h2>
                            <p class="section__desc text-white pb-4">
                                @foreach (array_chunk(explode(' ', trim($slider->description)), 12) as $index => $chunk)
                                    {!! $loop->first ? '' : '<br>' !!}{{ e(implode(' ', $chunk)) }}
                                @endforeach
                            </p>
