                            ...
                            <label class="custom-control-label custom--control-label" for="rating{{ $avg_rating }}">
                                <span class="rating-wrap d-flex align-items-center">
                                    ...
                                    <span class="rating-total pl-1">
                                        <span class="mr-1 text-black">{{ $avg_rating }} &
                                            up</span>({{ $count }})
                                    </span>
                                </span>
                            </label>
                ...
                <div class="card-body">
                    ...
                    @foreach ($durations as $duration)
                        ...
                        <div class="custom-control custom-checkbox mb-1 fs-15">
                            <input type="checkbox" class="custom-control-input" id="videoDuration{{ $loop->index }}"
                                name="duration[]" value="{{ $value }}">
                            <label class="custom-control-label custom--control-label text-black"
                                for="videoDuration{{ $loop->index }}">
                                {{ $label }}<span class="ml-1 text-gray">({{ $duration->count }})</span>
                            </label>
                        ...
                        @foreach ($languages->skip(4) as $language)
                            <div class="custom-control custom-checkbox mb-1 fs-15">
                                <input type="checkbox" class="custom-control-input"
                                    id="lang{{ $loop->parent->index + 4 }}" name="language[]"
                                    value="{{ $language->language }}">
                                <label class="custom-control-label custom--control-label text-black"
                                    for="lang{{ $loop->parent->index + 4 }}">
                                    {{ $language->language }}<span
                                        class="ml-1 text-gray">({{ $language->count }})</span>
                                </label>
                            </div><!-- end custom-control -->
                        @endforeach
                    </div><!-- end collapse -->
                ...
                <div class="card-body">
                    ...
                    @foreach ($cost as $c)
                        <div class="form-check mb-2">
                            <input class="form-check-input" type="radio" id="cost{{ $c->cost_type }}"
                                name="cost" value="{{ strtolower($c->cost_type) }}">
