                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <form class="delete-form"
                                                ...
                                                <button type="button" class="btn btn-primary px-2 ms-auto"
                                                    data-bs-toggle="modal" data-bs-target="#editSectionModal-{{ $section->id }}"> Edit
                                                    Section</button> &nbsp;
                                            ...
                                            <a class="btn btn-primary"
                                                onclick="addLectureDiv('{{ $course->slug }}', {{ $section->id }}, 'addLectureContainer{{ $key }}', {{ $key }})"
                                    ...
                                    <div class="container mb-3 " style="display: none;"
                                        ...
                                        <form method="POST" class="lecture-form" enctype="multipart/form-data">
                                            ...
                                            <div class="form-group col-md-12">
                                                ...
                                                <div class="card mb-3">
                                                    <div class="card-body">
                                                        <div class="row">
                                                            <div class="col-md-12">
                                                                <div id="lecture-upload-container{{ $key }}" class="text-center">
                                                                    <button type="button" id="browseLectureVideo{{ $key }}"
                                                                        class="btn btn-primary">Select Video</button>
                                                                    <input type="hidden" name="video_path"
                                                                        id="lecture_video_path{{ $key }}">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="row mt-3">
                                                            <div class="col-md-12">
                                                                <div id="lecture-upload-status{{ $key }}" class="alert"
                                            ...
                                            <textarea name="content" class="form-control mt-4 content" placeholder="Enter lecture content in case there is no video"
                                                rows="10" id="editor{{ $key }}"></textarea>
...
@section('scripts')
    ...
    <script>
        ...
        // Store Resumable.js instances to avoid memory leaks
        const resumableInstances = {};

        // ############## show lecture form ################
        function addLectureDiv(courseSlug, sectionId, containerId, key) {
            $(`#${containerId}`).show();
            const form = $(`#${containerId} form`);
            const url = `/instructor/courses/${courseSlug}/sections/${sectionId}/lectures`;
            form.attr('action', url);
            initializeResumable(key);
        }

        // Initialize Resumable.js for a specific lecture form
        function initializeResumable(key) {
            // Clean up previous instance if it exists
            if (resumableInstances[key]) {
                resumableInstances[key].cancel();
                delete resumableInstances[key];
            }
            ...
            resumableInstances[key] = lectureResumable;

            if (!lectureResumable.support) {
                showLectureUploadStatus(key, 'error', 'Your browser does not support resumable uploads');
                return;
            }

            const browseButton = document.getElementById(`browseLectureVideo${key}`);
            if (!browseButton) {
                console.error(`Browse button for key ${key} not found`);
                return;
            }

            lectureResumable.assignBrowse(browseButton);

            lectureResumable.on('fileAdded', function(file) {
                showLectureUploadStatus(key, 'info', `Preparing to upload: ${file.fileName}`);
                document.querySelector(`#addLectureContainer${key} .progress`).style.display = 'block';
                startLectureUpload(file, key);
            });

            lectureResumable.on('fileProgress', function(file) {
                const progress = Math.floor(file.progress() * 100);
                updateLectureProgress(progress, key);
            });

            lectureResumable.on('fileSuccess', function(file, message) {
                showLectureUploadStatus(key, 'success', `Upload complete: ${file.fileName}`);
            });

            lectureResumable.on('fileError', function(file, message) {
                showLectureUploadStatus(key, 'error', `Upload failed: ${message}`);
            });
        }

        async function startLectureUpload(file, key) {
            try {
                ...
                for (let chunkNumber = 1; chunkNumber <= totalChunks; chunkNumber++) {
                    ...
                    uploadedChunks++;
                    updateLectureProgress(Math.floor((uploadedChunks / totalChunks) * 100), key);
                }
                ...
                document.getElementById(`lecture_video_path${key}`).value = completeData.videoPath;
                showLectureUploadStatus(key, 'success', 'Video uploaded successfully');

            } catch (error) {
                console.error('Upload error:', error);
                showLectureUploadStatus(key, 'error', error.message);
            }
        }

        function updateLectureProgress(progress, key) {
            const progressBar = document.querySelector(`#addLectureContainer${key} .progress-bar`);
            progressBar.style.width = progress + '%';
            progressBar.setAttribute('aria-valuenow', progress);
            progressBar.textContent = progress + '%';
        }

        function showLectureUploadStatus(key, type, message) {
            const statusEl = document.getElementById(`lecture-upload-status${key}`);
        ...
        // Initialize Resumable.js for the first section on page load
        document.addEventListener('DOMContentLoaded', function() {
            @foreach ($sections as $key => $section)
                // Initialize only if the lecture form is visible
                if ($(`#addLectureContainer{{ $key }}`).is(':visible')) {
                    initializeResumable({{ $key }});
                }
            @endforeach
        });
